...
const PuzzleModule = (function() {
    ...
    /**
     * Generates a seed based on the current date
     * Ensures the same puzzle is generated for everyone on the same day
     */
     ...
     * from the complete puzzle solution
     */
    function createGameBoard(fullPuzzle, difficulty = 'easy') {
      ...
      const cellsToRemove = difficulties[difficulty] || difficulties.easy;
    ...
    return {
      ...
      selectCell: function(row, col) {
        if (row >= 0 && row < 9 && col >= 0 && col < 9) {
          ...
          const originalBoard = createGameBoard(puzzle);
      ...
      setCellValue: function(value) {
        if (selectedCell && value >= 0 && value <= 9) {
          ...
          return {
            ...
            value: value,
            conflict: hasConflict(gameBoard, selectedCell.row, selectedCell.col, value),
      ...
      /**
       * Check which cells are related to the given cell
       */
      getRelatedCells: function(row, col) {
        const related = [];
        
        for (let r = 0; r < 9; r++) {
          for (let c = 0; c < 9; c++) {
            if (areRelated(row, col, r, c) && !(r === row && c === col)) {
